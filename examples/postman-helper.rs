...
fn call(
    ...
    // headers
    let hdr0 = header_value(
        "Content-Type",
        "Content-Type",
        get_content_type(file_path).unwrap(),
    );
    let hdr1 = header_value(
        "Data-Usage-Agreement",
        "Data-Usage-Agreement",
        &format!("[{}]", &dua[0].serialize()),
    );
    let hdr2 = header_value(
        "Data-Tracker-Chain",
        "Data-Tracker-Chain",
        &base64::encode(&tracker.serialize()),
    );
    let hdr3 = header_value(
        "Authorization",
        "Author",
    ...
    rqst.insert("method", to_jsonvalue("POST"))
        .expect("Could not insert method!");
    rqst.insert("header", header)
        .expect("Could not insert header!");
    rqst.insert("body", gen_body(file_path))
        .expect("Could not insert body!");
    rqst.insert("url", gen_uri(url.clone()))
        .expect("Could not insert url!");
    ...
    itm.insert("response", rspns)
        .expect("Could not insert response!");
    item.push(itm).expect("Could not push itm!");

    collection
        .insert("info", gen_info(url))
        .expect("Could not set info!");
    collection
        .insert("item", item)
        .expect("Could not set item!");
    collection
        .insert("protocolProfileBehavior", JsonValue::new_object())
        .expect("Could not set protocolProfileBehavior!");
    ...
    match path.exists() {
        true => {
            ...
            body.insert("raw", String::from_utf8(data).unwrap())
                .expect("Could not insert raw!");
            body.insert(
                "options",
                json::parse(r#"{"raw":{"language":"json"}}"#).unwrap(),
            )
            .expect("Could not insert options!");
...
fn gen_info(url: Url) -> JsonValue {
    ...
    info.insert(
        "_postman_id",
        to_jsonvalue("c34879bc-68b0-4d47-a475-2e54d0f9ffe4"),
    )
    .expect("Could not set _postman_id!");
    info.insert("name", to_jsonvalue(&url.as_str()))
        .expect("Could not set name!");
    info.insert(
        "schema",
        to_jsonvalue("https://schema.getpostman.com/json/collection/v2.1.0/collection.json"),
    )
    .expect("Could not set schema!");
...
fn gen_uri(url: Url) -> JsonValue {
    ...
    uri.insert("raw", to_jsonvalue(url.as_str()))
        .expect("Could not set raw!");
    uri.insert("protocol", to_jsonvalue("http"))
        .expect("Could not set protocol!");
    let mut hosts = JsonValue::new_array();
    hosts
        .push(url.host_str().unwrap())
        .expect("Could not push hosts!");
    ...
    {
        params
            .push(to_jsonvalue(param))
            .expect("Could not push parameters!");
    }
...
fn header_value(key: &str, name: &str, value: &str) -> JsonValue {
    ...
    hdr.insert("key", to_jsonvalue(key))
        .expect("Could not set header key!");
    hdr.insert("name", to_jsonvalue(name))
        .expect("Could not set header name!");
    hdr.insert("value", to_jsonvalue(value))
        .expect("Could not set header value!");
    hdr.insert("type", to_jsonvalue("text"))
        .expect("Could not set header type!");
...
fn main() {
    ...
    let usg_agree =
        input_from_user("What is the name of the usage agreement, (e.g.: For Billing Purpose)?");
    let usg_agree_uri = input_from_user(
        "Where is the usage agreement located, (e.g.: https://www.dua.org/billing.pdf)?",
    );
    let url = Url::parse(&usg_agree_uri).unwrap();
    let mut duas = Vec::new();
    duas.push(DUA::new(
        usg_agree.trim().to_string(),
